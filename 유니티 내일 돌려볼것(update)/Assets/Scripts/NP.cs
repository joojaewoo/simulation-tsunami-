...
public class NP : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        destnation= GameObject.FindGameObjectsWithTag("Finish");
        dist = new double[destnation.Length];
        agent = GetComponent<NavMeshAgent>();
        agent.autoBraking = false;
        anim = GetComponent<Animator>();
        rb = GetComponent<Rigidbody>();
        color = agent.GetComponentInChildren<SkinnedMeshRenderer>();
        SetState(State.Walk);
        sw.Start();
    }
    ...
    {
        j = 0;
        for (int i = 0; i < destnation.Length; i++) // 에이전트와 각각의 목적지와의 거리를 계산하여 dist 배열에 저장
        {
            dist[i] = Vector3.Distance(agent.transform.position, destnation[i].transform.position);
        }
        double min1 = dist[0];
    ...
    void SetDest()
    {
        //agent.SetDestination(destnation[j].transform.position); // 목적지 지정
        agent.destination = destnation[j].transform.position;
    }
